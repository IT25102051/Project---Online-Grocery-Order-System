...
<%
    ...
    // Load existing notifications: id,recipient,title,message,timestamp,read
    ...
    if (nf.exists()) {
        ...
        while ((line = br.readLine()) != null) {
            ...
            notifs.add(parts);
            if (parts.length >= 6 && !"read".equalsIgnoreCase(parts[5].trim())) {
                unreadNotifications++;
            }
        }
        br.close();
    ...
    // Find notification to edit or view
    String[] editNotif = null;
    String[] viewNotif = null;
    ...
    if (viewId != null) {
        for (String[] n : notifs) {
            if (n.length >= 4 && n[0].equals(viewId)) {
                viewNotif = n;
                break;
            }
        }
    ...
    <div class="content">
        <div class="stat-grid">
            ...
            <div class="stat-card">
                ...
                <div class="stat-num"><%= unreadNotifications %></div>
        ...
        <div class="layout-grid">
            <div class="panel">
                ...
                <div class="panel-body">
                        ...
                        <% } else if ("ok".equals(updated)) { %>
                        <div class="success-message">
                            <i class="fa-solid fa-check-circle"></i> Notification updated successfully.
                        </div>
                        <% } else if ("ok".equals(deleted)) { %>
                        <div class="success-message">
                            <i class="fa-solid fa-check-circle"></i> Notification deleted successfully.
                        </div>
                        <% } %>

                        <% if (viewNotif != null) {
                            String vtitle = viewNotif.length > 2 ? viewNotif[2].replace(";", ",") : "—";
                            String vmsg = viewNotif.length > 3 ? viewNotif[3].replace(";", ",") : "—";
                            String vtime = viewNotif.length > 4 ? viewNotif[4] : "—";
                            String vrecip = viewNotif.length > 1 ? viewNotif[1] : "All";
                        %>
                        <div style="margin-bottom:18px; border:1px solid #e6eef6; border-radius:12px; padding:14px; background:#fff;">
                            <div style="display:flex; gap:12px; align-items:flex-start;">
                                <div style="width:44px; height:44px; background:#16a34a; color:#fff; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:16px;">
                                    <i class="fa-solid fa-bell"></i>
                                </div>
                                <div style="flex:1;">
                                    <div style="font-weight:700; color:#0f172a; margin-bottom:6px;"><%= vtitle %></div>
                                    <div style="color:#64748b; margin-bottom:8px;"><%= vmsg %></div>
                                    <div style="font-size:12px; color:#94a3b8;">Recipient: <%= vrecip %> · Sent: <%= vtime %></div>
                                </div>
                            </div>
                        </div>
                        <% } %>
                        ...
                        <div class="form-group">
                            ...
                            <select name="recipient" required>
                                <option value="All" <%= editNotif == null || editNotif.length < 2 || "All".equalsIgnoreCase(editNotif[1]) ? "selected" : "" %>>All Users</option>
                                <option value="Customer" <%= editNotif != null && editNotif.length > 1 && "Customer".equalsIgnoreCase(editNotif[1]) ? "selected" : "" %>>Customers</option>
                                <option value="Delivery" <%= editNotif != null && editNotif.length > 1 && "Delivery".equalsIgnoreCase(editNotif[1]) ? "selected" : "" %>>Delivery Staff</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label>Notification Title</label>
                            <input type="text" name="title" placeholder="e.g. Weekend Sale Alert!" value="<%= editNotif != null ? editNotif[2].replace(";", ",") : "" %>" required>
                        </div>
                        <div class="form-group">
                            <label>Message Content</label>
                            <textarea name="message" placeholder="Enter your notification message here..." required><%= editNotif != null ? editNotif[3].replace(";", ",") : "" %></textarea>
                        </div>
            ...
            <div class="panel">
                ...
                <% } else { %>
                    <div class="notification-list">
                        <div style="margin-bottom:16px; font-weight:700; color:#0f172a;">General / Customer Notifications</div>
                        <% boolean hasGeneral = false; %>
                        <% for (String[] n : notifs) {
                            String recipient = n.length > 1 ? n[1] : "All";
                            if ("Delivery".equalsIgnoreCase(recipient)) continue;
                            hasGeneral = true;
                            String nid = n.length > 0 ? n[0] : "";
                            String ntitle = n.length > 2 ? n[2].replace(";", ",") : "—";
                            String nmsg = n.length > 3 ? n[3].replace(";", ",") : "—";
                            String ntime = n.length > 4 ? n[4] : "—";
                            String status = n.length > 5 ? n[5] : "unread";
                        %>
                        <div class="notification-item" style="border-color:<%= "read".equalsIgnoreCase(status) ? "#d1fae5" : "#fecdd3" %>">
                            ...
                            <div class="notification-content">
                                ...
                                <div class="notification-meta">Recipient: <%= recipient %> · Sent: <%= ntime %></div>
                            </div>
                            <div class="action-buttons">                                <a href="adminNotifications.jsp?view=<%= nid %>" class="btn btn-secondary">
                                    <i class="fa-solid fa-eye"></i> View
                                </a>                                <a href="adminNotifications.jsp?edit=<%= nid %>" class="btn btn-edit">
                        ...
                        <% } %>
                        <% if (!hasGeneral) { %>
                            <div class="empty-state">No general or customer notifications available.</div>
                        <% } %>

                        <div style="margin:24px 0 16px 0; font-weight:700; color:#0f172a;">Delivery Notifications</div>
                        ...
                        <div class="notification-item" style="border-color:<%= "read".equalsIgnoreCase(status) ? "#d1fae5" : "#fecdd3" %>">
                            ...
                            <div class="action-buttons">
                                <a href="adminNotifications.jsp?view=<%= nid %>" class="btn btn-secondary">
                                    <i class="fa-solid fa-eye"></i> View
                                </a>
                        ...
                        <% } %>
                        <% if (!hasDelivery) { %>
                            <div class="empty-state">No delivery notifications have been sent yet.</div>
                        <% } %>
                    </div>
                <% } %>
                </div>
